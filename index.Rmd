...
map2(links2,filenames2,download_file)
...
forcefile_reader <- function(file) {
  name2 = str_split_fixed(file, '/', 2)[, 2] %>%
    gsub('SBB_', '', .)
  df <- read.csv(file2, header = FALSE, sep = '') %>%
    select(V1,V2,V3,V7,V10)%>% 
    rename(year=1,month=2,day=3,precip=4,airtemp=5)%>%
    mutate(site = name2) 
...
ggplot(temp_yearly, aes(x=year, y=mean_yrtemp, color=site))+
  geom_line()+ 
  theme_few()+ 
  scale_color_manual(name="Site", values= c("#274a12","#babf28"))+ 
  labs(x='Year', y='Average Air Temperature')+ 
  xlim(2001,2012)
```
...
```{r}
# Create data frame for monthly average temp per year per site 
temp_monthly <- forcing_data_full %>% 
  group_by(year, month, site)%>% 
  summarize(mean_motemp=mean(airtemp, na.rm=T))

ggplot(temp_monthly,aes(x=year,y=mean_motemp,color=site))+
    geom_line()

# Write function to make line plots of monthly average temps per site per year 
years = list[]
line_plotter <- function(year){
  ggplot(temp_monthly,aes(x=year,y=mean_notemp,color=site))+
    geom_line()
}
...
## Create a plot of average daily precip by day of year, distinguishing between sites 

```{r}
library(tidyr)

# Create data frame with mean daily precip by day of year
precip_daily <- forcing_data_full %>% 
  tidyr::unite(DOM,c(month,day), sep="-", remove=FALSE) %>% 
...
## Create yearly plots of precip by day of year, distinguishing between sites 

```{r}
# Use function and for loop to create yearly plots of precip by day of year, color by site 
# ggplot(x=,y=,)
# 
