...
line_plotter <- function(df, year) {
  temp_monthly <- df %>%
    ...
    print(ggplot(temp_monthly, aes(x = month, y = mean_motemp, color = site)) +
    geom_line() +
    theme_few() +
    labs(x = 'Month', y = 'Average Air Temperature', title = i))
...
precip_daily <- forcing_data_full %>% 
  ...
  summarize(mean_precip= mean(precip, na.rm=T)) %>%
  mutate(DoM = paste(month,day, sep= "/"))%>% 
  pivot_wider(names_from = site,values_from=mean_precip)%>% 
  select(-SBSP)
...
ggplot(precip_daily, aes(x = DoM, y = SASP)) +
    geom_point(shape=1) +
    theme_few() +
    labs(x = 'Day of Year', y = 'Average Precipitation')
    
  
>>>>>>> 02944085798f638d3e4b1f3adf5e07698227c66b
```
...
# Write function to create yearly plots of precipitation by day of year 

precip_plotter <- function(df, year) {
<<<<<<< HEAD
  precip_daily3 <- df %>%
=======
  precip_daily2 <- df %>%
...
=======
  ...
  print(
    ggplot(precip_daily2, aes(x = date, y = SASP)) +
      geom_point(shape = 1) +
      theme_few() +
      labs(x = 'Day of Year', y = 'Average Precipitation', title = i)
  )
}
...
for (i in yrs){
  precip_plotter(forcing_data_full,year)
}
<<<<<<< HEAD
=======
...
# precip_xts <- xts(precip_daily2 %>%
#                     select(SASP,-month), order.by=precip_daily2$date)
# 
# dygraph(precip_xts)
>>>>>>> 02944085798f638d3e4b1f3adf5e07698227c66b
```
